...
# pragma omp parallel for
    ...
    for (i = 0; i < nelem; i++)
    {
        ...
        if (elem[i].lc.glacier == 1 && elem[i].ps.iceh <= 0.0)
        {
            ...
            _InitLc(&elem[i], lctbl, cal, SIMPLE_URBAN);
        }
...
#endif
{
    ...
    if (lc->isurban)
    {
        ...
        epc->rsmin = 400.0;
#endif
        /* In original Noah LSM, simple urban model changes urban soil porosity.
         * In Flux-PIHM, porosity should not be changed because it is also used
         * in PIHM hydrology calculation. In addition, field capacity and
         * wilting point are not changed to allow transpiration from the
         * non-urban fraction. Parameter smcdry is adjusted according to Noah
         * urban smcdry saturation level to limit direct evaporation from non-
         * vegetated area, which includes urban area */
