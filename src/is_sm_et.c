...
void IntcpSnowET (int t, double stepsize, pihm_struct pihm)
{
    ...
    for (i = 0; i < pihm->numele; i++)
    {
        ...
        pres = 101.325 * 1.0e3 * pow((293.0 - 0.0065 * elem->topo.zmax) / 293.0, 5.26);
        qv = 0.622 * vp / pres;
        ...
        frac_snow = sfctmp < TSNOW ? 1.0 : sfctmp > TRAIN ? 0 : (TRAIN - sfctmp) / (TRAIN - TSNOW);
         ...
         * elemental area */
        snow_grnd = snow_grnd + (1.0 - elem->lc.vegfrac) * snow_rate * stepsize;
        snow_canopy = snow_canopy + elem->lc.vegfrac * snow_rate * stepsize;
        snow_intcp_max = snow_canopy > 0.0 ? 0.003 * lai * elem->lc.vegfrac : 0.0;
        if (snow_canopy > snow_intcp_max)
        {
            snow_grnd += snow_canopy - snow_intcp_max;
            snow_canopy = snow_intcp_max;
        }
        melt_rate_grnd = melt_rate_canopy = (sfctmp > T0 ? (sfctmp - T0) * meltf : 0.0);
        ...
        ra = log (elem->forc.zlvl_wind / z0) * log (10.0 * elem->forc.zlvl_wind / z0) / (wind * 0.16);
        ...
        if (lai > 0.0)
        {
            elem->et[0] = elem->lc.vegfrac * (pow ((elem->intcp < 0.0 ? 0.0 : (elem->intcp > intcp_max ? intcp_max : elem->intcp)) / intcp_max, elem->lc.cfactr)) * etp;
            ...
            etas = 1.0 - 0.0016 * (pow ((elem->lc.topt - 273.15 - sfctmp), 2));
            ...
            elem->et[1] = elem->lc.vegfrac * pc * (1.0 - pow(((elem->intcp + snow_canopy < 0.0) ? 0.0 : (elem->intcp + snow_canopy)) / (intcp_max + snow_intcp_max), elem->lc.cfactr)) * etp;
            elem->et[1] = elem->et[1] < 0.0 ? 0.0 : elem->et[1];
            elem->et[1] = ((elem->gw < (elem->soil.depth - elem->lc.rzd)) && elem->unsat <= 0.0) ? 0.0 : elem->et[1];

            elem->drip = elem->intcp <= 0.0 ? 0.0 : 5.65e-2 * intcp_max * exp (3.89 * (elem->intcp < 0.0 ? 0.0 : elem->intcp) / intcp_max);
        }
        else
        {
            elem->et[1] = 0.0;
        ...
        {
            if (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) >= elem->et[0] + elem->drip)
            {
                ret = elem->drip + (elem->intcp - intcp_max)/stepsize + (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - (elem->et[0] + elem->drip));
                isval = intcp_max;
            }
            else if ((((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) < elem->et[0] + elem->drip) && (elem->intcp + stepsize * ((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy - elem->et[0] - elem->drip) <= 0.0))
            {
                elem->et[0] = (elem->et[0] / (elem->et[0] + elem->drip)) * (elem->intcp / stepsize + ((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy));
                ret = (elem->drip / (elem->et[0] + elem->drip)) * (elem->intcp / stepsize + ((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy));
                isval = 0.0;
            }
            else
            {
                isval = elem->intcp + stepsize * (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - elem->et[0] - elem->drip);
                ret = elem->drip;
            }
        }
        else if ((elem->intcp < intcp_max) && ((elem->intcp + (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - elem->et[0] - elem->drip) * stepsize) >= intcp_max))
        {
            isval = intcp_max;
            ret = elem->drip + (((elem->intcp + (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - elem->et[0] - elem->drip) * stepsize) - intcp_max))/stepsize;
        }
        else if ((elem->intcp < intcp_max) && ((elem->intcp + (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - elem->et[0] - elem->drip) * stepsize) <= 0.0))
        {
        ...
        {
            isval = elem->intcp + (((1.0 - frac_snow) * elem->prcp * elem->lc.vegfrac + melt_rate_canopy) - elem->et[0] - elem->drip) * stepsize;
            ret = elem->drip;
        }
    
        elem->netprcp = (1.0 - elem->lc.vegfrac) * (1.0 - frac_snow) * elem->prcp + ret + melt_rate_grnd;
            elem->drip = ret;
            elem->intcp = isval;
    }
}
