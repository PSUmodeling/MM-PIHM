...
int f (realtype t, N_Vector CV_Y, N_Vector CV_Ydot, void *DS)
{
    ...
    for (i = 0; i < MD->NumEle; i++)
    {
        ...
        if (MD->DummyY[i + 2 * MD->NumEle] > AquiferDepth - MD->Ele[i].infD)
        {
            ...
            DY[i + MD->NumEle] =
               DY[i + MD->NumEle] + MD->EleViR[i] - MD->Recharge[i];
#ifdef _FLUX_PIHM_
            DY[i + 2 * MD->NumEle] =
               DY[i + 2 * MD->NumEle] + MD->Recharge[i] - MD->EleET[i][2];
#else
            DY[i + 2 * MD->NumEle] =
               DY[i + 2 * MD->NumEle] + MD->Recharge[i] - (MD->DummyY[i] <
               (EPS / 100) ? MD->EleET[i][2] : 0);
#endif
        }
        else
        {
            ...
            DY[i + MD->NumEle] =
               DY[i + MD->NumEle] + MD->EleViR[i] - MD->Recharge[i] -
               ((MD->DummyY[i] < EPS / 100) ? MD->EleET[i][2] : 0);
#endif
        ...
        DY[i + 2 * MD->NumEle] =
           DY[i + 2 * MD->NumEle] - MD->EleETsat[i] * MD->EleET[i][1];
        DY[i + MD->NumEle] =
           DY[i + MD->NumEle] - (1 - MD->EleETsat[i]) * MD->EleET[i][1];
#else
        DY[i] =
           DY[i] + MD->EleNetPrep[i] - MD->EleViR[i] - ((MD->DummyY[i] <
              EPS / 100) ? 0 : MD->EleET[i][2]);
    ...
    for (i = 0; i < MD->NumRiv; i++)
    {
        for (j = 0; j <= 6; j++)
        {
            ...
            DY[i + 3 * MD->NumEle] =
               DY[i + 3 * MD->NumEle] -
               MD->FluxRiv[i][j] / (MD->Riv[i].Length *
               CS_AreaOrPerem (MD->Riv_Shape[MD->Riv[i].shape - 1].interpOrd,
                  MD->Riv[i].depth, MD->Riv[i].coeff, 3));
        }
