...
# pragma omp parallel for
    ...
    for (i = 0; i < nriver; i++)
    {
        ...
        perim =
            RiverPerim(riv[i].shp.intrpl_ord, riv[i].ws.stage, riv[i].shp.coeff);
        ...
        if (riv[i].down > 0)
        {
            ...
            crossa =
                RiverArea(riv[i].shp.intrpl_ord, riv[i].ws.stage,
                riv[i].shp.coeff);
            crossa_down =
        ...
        else
        {
            switch (riv[i].down)
            {
                case DIRICHLET:
                    ...
                    avg_perim = perim;
                    crossa = RiverArea(riv[i].shp.intrpl_ord, riv[i].ws.stage,
                        riv[i].shp.coeff);
