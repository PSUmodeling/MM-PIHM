    ...
    elem_struct *elem_ptr)
{
    ...
    {
        pihm_printf(VL_ERROR, "Error: Land cover type %d for Element %d "
    ...
    elem_ptr->lc.bare = (IGBP_BARREN == elem_ptr->attrib.lc ||
        NLCD40_BARREN == elem_ptr->attrib.lc) ? 1 : 0;
    elem_ptr->lc.shdfac = (1 == elem_ptr->lc.bare) ? 0.0 : elem_ptr->lc.shdfac;
#if defined(_NOAH_)
    elem_ptr->lc.snup = lctbl->snup[lc_ind];
    elem_ptr->lc.isurban = (IGBP_URBAN_BUILDUP == elem_ptr->attrib.lc ||
        NLCD40_DEVELOPED_OPEN == elem_ptr->attrib.lc ||
        NLCD40_DEVELOPED_LOW == elem_ptr->attrib.lc ||
        NLCD40_DEVELOPED_MID == elem_ptr->attrib.lc ||
        NLCD40_DEVELOPED_HIGH == elem_ptr->attrib.lc) ? 1 : 0;
    elem_ptr->lc.glacier =
        (IGBP_SNOW_ICE == elem_ptr->attrib.lc ||
        NLCD40_SNOW_ICE == elem_ptr->attrib.lc) ? 1 : 0;
