...
int bgc (bgcin_struct * bgcin, bgcout_struct * bgcout, int mode)
{
    ...
    if (ok && output_map_init (output_map, &metv, &ws, &wf, &cs, &cf, &ns, &nf, &phen, &epv, &psn_sun, &psn_shade, &summary))
    {
    ...
    {
        if (ok && restart_input (&ctrl, &ws, &cs, &ns, &epv, &metyr, &(bgcin->restart_input)))
        {
    ...
    do
    {
        ...
        for (simyr = 0; ok && simyr < tmpyears; simyr++)
        {
            ...
            if (metyr == ctrl.metyears)
            {
                ...
                {
                    if (ctrl.onscreen)
                        bgc_printf (BV_DETAIL, "Resetting met data for cyclic input\n");
                }
                if (mode == MODE_SPINUP)
                {
            ...
            if (mode == MODE_MODEL)
            {
                ...
                else
                {
                    ...
                    if (ramp_ndep.doramp && !bgcin->ndepctrl.varndep)
                    {
                        ...
                        ndep_scalar = (ramp_ndep.ind_ndep - ramp_ndep.preind_ndep) / (co2.co2ppm_array[ind_simyr] - co2.co2ppm_array[0]);
                        ndep_diff = (co2.co2ppm_array[simyr] - co2.co2ppm_array[0]) * ndep_scalar;
            ...
            for (yday = 0; ok && yday < 365; yday++)
            {
                ...
                if (ok && daily_nitrogen_state_update (&nf, &ns, annual_alloc, epc.woody, epc.evergreen))
                {
            ...
            /*******************/
                ...
                if (ctrl.domonavg)
                {
                    ...
                    if (yday == endday[curmonth])
                    {
                        ...
                        /* write to file */
                        if (fwrite (monavgarr, sizeof (float), ctrl.ndayout, bgcout->monavgout.ptr) != (size_t) ctrl.ndayout)
                        {
                ...
                if (ctrl.doannavg)
                {
                    ...
                    if (yday == 364)
                    {
                        ...
                        /* write to file */
                        if (fwrite (annavgarr, sizeof (float), ctrl.ndayout, bgcout->annavgout.ptr) != (size_t) ctrl.ndayout)
                        {
                ...
                if (mode == MODE_MODEL)
                {
                    ...
                    annet += wf.canopyw_evap + wf.snoww_subl + wf.soilw_evap + wf.soilw_trans;
            ...
            if (ok && ctrl.doannual)
            {
                ...
                if (fwrite (annarr, sizeof (float), ctrl.nannout, bgcout->annout.ptr) != (size_t) ctrl.nannout)
                {
    ...
    } while (mode == MODE_SPINUP && (!(steady1 && steady2) && (spinyears < ctrl.maxspinyears || metcycle != 0)));
