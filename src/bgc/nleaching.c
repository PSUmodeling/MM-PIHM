...
void NLeaching (elem_struct *elem, int numele, river_struct *riv, int numriv)
{
    ...
    for (i = 0; i < numele; i++)
    {
        totwater = (elem[i].daily.ws.unsat * elem[i].soil.porosity +
    ...
    for (i = 0; i < numele; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            latflux = elem[i].daily.wf.subsurf[j] * 24.0 * 3600.0 * 1000.0 /
                elem[i].topo.area;
    ...
    for (i = 0; i < numriv; i++)
    {
        ...
        latflux = (riv[i].daily.wf.river[LEFT_AQUIF2CHANL] +
        ...
        latflux = riv[i].daily.wf.river[RIGHT_AQUIF2CHANL] +
