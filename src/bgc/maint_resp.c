    ...
    epvar_struct *epv, cflux_struct *cf)
{
    ...
    if (cs->leafc)
    {
        ...
        // Convert to respiration flux in kg C/m2 projected area/day, and correct for temperature
        dlmr_area_sun = n_area_sun * MRPERN * pow(Q10, exponent);
        dlmr_area_shade = n_area_shade * MRPERN * pow(Q10, exponent);

        // Finally, convert from mass to molar units, and from a daily rate to a rate per second
    ...
    // Amended to consider only the specified n concentration, to avoid excessive MR with n-loading to fine roots
    if (cs->frootc)
    {
