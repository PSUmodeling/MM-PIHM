...
void TotalPhotosynthesis(const epconst_struct *epc, const daily_struct *daily, const phystate_struct *ps,
    epvar_struct *epv, cflux_struct *cf, psn_struct *psn_sun, psn_struct *psn_shade)
{
    // This function is a wrapper and replacement for the photosynthesis code which used to be in the central bgc.c
    // code. At Mott Jolly's request, all of the science code is being moved into functions.
    double          tday;
    ...
    // sum multiplied by the projected leaf area in the relevant canopy fraction, and this total converted from
    // umol/m2/s -> kgC/m2/d
    cf->psnsun_to_cpool = (epv->assim_sun + epv->dlmr_area_sun) * ps->plaisun * epv->dayl * 12.011E-9;
    ...
    // Convert conductance from m/s --> umol/m2/s/Pa, and correct for CO2 vs. water vapor
    psn_shade->g = epv->gl_t_wv_shade * 1.0E6 / (1.6 * 8.3143 * (tday + TFREEZ));
    ...
    // sum multiplied by the projected leaf area in the relevant canopy fraction, and this total converted from
    // umol/m2/s -> kgC/m2/d
    cf->psnshade_to_cpool = (epv->assim_shade + epv->dlmr_area_shade) * ps->plaishade * epv->dayl * 12.011E-9;
}
...
void Photosynthesis(psn_struct *psn)
{
    ...
    // The weight proportion of Rubisco to its nitrogen content, fnr, is calculated from the relative proportions of the
    // basic amino acids that make up the enzyme, as listed in the Handbook of Biochemistry, Proteins, Vol III, p. 510,
    // which references: Kuehn and McFadden, Biochemistry, 8:2403, 1969
    static double   fnr = 7.16;             // kg Rub/kg NRub

    // The following enzyme kinetic constants are from: Woodrow, I.E., and J.A. Berry, 1980. Enzymatic regulation of
    // photosynthetic CO2 fixation in C3 plants. Ann. Rev. Plant Physiol. Plant Mol. Biol., 39:533-594.
    // Note that these values are given in the units used in the paper, and that they are converted to units appropriate
    // to the rest of this function before they are used.
    // I've changed the values for Kc and Ko from the Woodrow and Berry reference, and am now using the values from
    // De Pury and Farquhar, 1997.
    // Simple scaling of photosynthesis from leaves to canopies without the errors of big-leaf models. Plant, Cell and
    // Env. 20: 537-557.
    // All other parameters, including the q10's for Kc and Ko are the same as in Woodrow and Berry.
    ...
    // Wullschleger, S.D., 1993.  Biochemical limitations to carbon assimilation in C3 plants - A retrospective analysis
    // of the A/Ci curves from 109 species. Journal of Experimental Botany, 44:907-920.
    ...
    // Solve for Av and Aj using the quadratic equation, substitution for Ci from A = g(Ca-Ci) into the equations from
