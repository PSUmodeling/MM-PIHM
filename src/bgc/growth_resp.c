...
void GrowthResp(const epconst_struct *epc, cflux_struct *cf)
{
    ...
    if (epc->woody)
    {
        ...
        cf->cpool_livestem_storage_gr = cf->cpool_to_livestemc_storage *
            g1 * g2;
        cf->cpool_deadstem_storage_gr = cf->cpool_to_deadstemc_storage *
            g1 * g2;
        cf->cpool_livecroot_storage_gr = cf->cpool_to_livecrootc_storage *
            g1 * g2;
        cf->cpool_deadcroot_storage_gr = cf->cpool_to_deadcrootc_storage *
            g1 * g2;
        cf->transfer_livestem_gr = cf->livestemc_transfer_to_livestemc *
            g1 * (1.0 - g2);
        cf->transfer_deadstem_gr = cf->deadstemc_transfer_to_deadstemc *
            g1 * (1.0 - g2);
        cf->transfer_livecroot_gr = cf->livecrootc_transfer_to_livecrootc *
            g1 * (1.0 - g2);
        cf->transfer_deadcroot_gr = cf->deadcrootc_transfer_to_deadcrootc *
            g1 * (1.0 - g2);
    }
}
