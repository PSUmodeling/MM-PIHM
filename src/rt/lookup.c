...
void Lookup(FILE *fp, const calib_struct *calib, chemtbl_struct chemtbl[], kintbl_struct kintbl[], rttbl_struct *rttbl)
{
    ...
    {
        pihm_printf(VL_ERROR, "Error reading temperature points in %s near Line %d", ".cdbs", lno);
    ...
    {
        pihm_printf(VL_ERROR, "Error reading Debye Huckel parameters in %s near Line %d", ".cdbs", lno);
        pihm_exit(EXIT_FAILURE);
    }
    ...
    pihm_printf(VL_VERBOSE, " Debye-Huckel Parameters set to A=%6.4f; B=%6.4f; b=%6.4f\n\n",
    ...
    for (i = 0; i < rttbl->num_mkr + rttbl->num_akr; i++)
    {
        ...
        pihm_printf(VL_VERBOSE, " Selecting the kinetic species %s from all possible species.\n\n",
    ...
    for (i = 0; i < rttbl->num_mkr; i++)
    {
        ...
        {
            pihm_printf(VL_ERROR, "Error finding mineral kinetic %s -label %s in the database.\n",
    ...
    if (rttbl->num_ssc > 0)
    {
        ...
        {
            pihm_printf(VL_VERBOSE, " %-14s", chemtbl[i + rttbl->num_stc].name);
            for (j = 0; j < rttbl->num_sdc; j++)
            {
    ...
    for (j = 0; j < rttbl->num_mkr + rttbl->num_akr; j++)
    {
        pihm_printf(VL_VERBOSE, " %-14s", chemtbl[j + rttbl->num_spc + rttbl->num_ads + rttbl->num_cex].name);
        for (i = 0; i < rttbl->num_stc; i++)
        {
...
void ReadPrimary(const char cmdstr[], int num_stc, chemtbl_struct chemtbl[])
{
    ...
    for (i = 0; i < num_stc; i++)
    {
        if (MatchWrappedKey(cmdstr, chemtbl[i].name) == 0)
        {
            ...
            {
                pihm_printf(VL_ERROR, "Error reading primary species parameters for %s\n", chemtbl[i].name);
                pihm_exit(EXIT_FAILURE);
            }
            pihm_printf(VL_VERBOSE, " Primary species %s found in database.\n molar_mass = %6.4f\n\n",
...
void ReadSecondary(const char cmdstr[], int npoints, int keq_position, chemtbl_struct chemtbl[], rttbl_struct *rttbl)
{
    ...
    for (i = 0; i < rttbl->num_ssc; i++)
    {
        ...
        {
            pihm_printf(VL_VERBOSE, " Secondary species %s found in database!\n", chemtbl[ind].name);
            ...
            pihm_printf(VL_VERBOSE, " molar_mass = %6.4f, Charge = %6.4f, SizeFactor = %6.4f\n\n",
    ...
    double keq_kin_all[], chemtbl_struct chemtbl[], rttbl_struct *rttbl)
{
    ...
    for (i = 0; i < rttbl->num_min; i++)
    {
        ...
        if (MatchWrappedKey(chemn, chemtbl[ind].name) == 0)
        {
            ...
            pihm_printf(VL_VERBOSE, " molar_mass = %6.4f, molar_vol = %6.4f\n\n",
...
void ReadAdsorption(const char cmdstr[], int npoints, int keq_position, chemtbl_struct chemtbl[], rttbl_struct *rttbl)
{
    ...
    for (i = 0; i < rttbl->num_ssc; i++)
    {
        ...
        {
            pihm_printf(VL_VERBOSE, " Secondary surface complexation %s found in database!\n", chemtbl[ind].name);
            pihm_printf(VL_VERBOSE, " %s", cmdstr);
...
void ReadCationEchg(const char cmdstr[], double calval, chemtbl_struct chemtbl[], rttbl_struct *rttbl)
{
    ...
    for (i = 0; i < rttbl->num_ssc; i++)
    {
        ...
        if (strcmp(chemtbl[ind].name, chemn) == 0)
        {
            pihm_printf(VL_VERBOSE, " Secondary ion exchange %s found in database!\n", chemtbl[ind].name);
            pihm_printf(VL_VERBOSE, " %s", cmdstr);
    ...
    kintbl_struct *kintbl)
{
    ...
    if (strcmp(chemtbl[kintbl->position].name, chemn) == 0)
    {
        ...
        if (strcmp(kintbl->label, label) == 0)
        {
            ...
            {
                pihm_printf(VL_VERBOSE, "Error: Cannot find mineral kinetics type in .cdbs near Line %d.\n");
                pihm_exit(EXIT_FAILURE);
            }
            ...
            if (strcmp(optstr, "rate(25C)") == 0)
            {
                ...
                kintbl->rate += calval;
                pihm_printf(VL_VERBOSE, " After calibration: Rate is %f, calib->Rate = %f \n", kintbl->rate, calval);
            }
            else
            {
                pihm_printf(VL_VERBOSE, "Error: Cannot find mineral kinetics rate in .cdbs near Line %d.\n");
                pihm_exit(EXIT_FAILURE);
            }
            ...
            {
                pihm_printf(VL_VERBOSE, "Error: Cannot find mineral kinetics activation in .cdbs near Line %d.\n");
                pihm_exit(EXIT_FAILURE);
            }
            ...
            while (cmdstr[0] != '+')
            {
                ...
                if (strcmp(optstr, "dependence") == 0)
                {
                    ...
                    {
                        wrap(chemn);
                        kintbl->dep_index[0] = FindChem(chemn, chemtbl, num_stc);
                        if (kintbl->dep_index[0] < 0)
                        {
                            pihm_printf(VL_ERROR, "Error finding dependence in species table.\n");
                            pihm_exit(EXIT_FAILURE);
                        }
                        kintbl->ndep = 1;
                ...
                else if (strcmp(optstr, "biomass") == 0)
                {
                    ...
                    kintbl->biomass_index = FindChem(chemn, chemtbl, num_stc);
                    pihm_printf(VL_VERBOSE, " Biomass species position: %d \n",
                ...
                else if (strcmp(optstr, "monod_terms") == 0)
                {
                    ...
                    while (sscanf(cmdstr + bytes_consumed, "%s %lf%n", chemn, &temp, &bytes_now) == 2)
                    {
                        ...
                        wrap(chemn);
                        kintbl->monod_index[kintbl->nmonod] = FindChem(chemn, chemtbl, num_stc);
                        if (kintbl->monod_index[kintbl->nmonod] < 0)
                        {
                            pihm_printf(VL_ERROR, "Error finding monod_terms in species table.\n");
                            pihm_exit(EXIT_FAILURE);
                        }
                        kintbl->monod_para[kintbl->nmonod] = temp;
                        pihm_printf(VL_VERBOSE, " Monod term: %s %f\n", chemn, kintbl->monod_para[kintbl->nmonod]);
                        kintbl->nmonod++;
                    }
                }
                else if (strcmp(optstr, "inhibition") == 0)
                {
                    ...
                    while (sscanf(cmdstr + bytes_consumed, "%s %lf%n", chemn, &temp, &bytes_now) == 2)
                    {
                        ...
                        wrap(chemn);
                        kintbl->inhib_index[kintbl->ninhib] = FindChem(chemn, chemtbl, num_stc);
                        if (kintbl->inhib_index[kintbl->ninhib] < 0)
                        {
                            pihm_printf(VL_ERROR, "Error finding inhibition term in species table.\n");
                            pihm_exit(EXIT_FAILURE);
                        }
                        kintbl->inhib_para[kintbl->ninhib] = temp;
                        pihm_printf(VL_VERBOSE, " Inhibition term: %s %f\n", chemn, kintbl->inhib_para[kintbl->ninhib]);
                        kintbl->ninhib++;
                    }
                }
