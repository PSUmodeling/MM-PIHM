...
void ReadCini(const char fn[], const chemtbl_struct *chemtbl, int num_stc, atttbl_struct *atttbl,
    chmictbl_struct *chmictbl)
{
    ...
    for (i = 0; i < nelem; i++)
    {
        ...
        match = sscanf(cmdstr, "%d %d %d %d",
            &index, &atttbl->prcpc[i], &atttbl->chem_ic[i][SOIL_CHMVOL], &atttbl->chem_ic[i][GEOL_CHMVOL]);
#else
        match = sscanf(cmdstr, "%d %d %d", &index, &atttbl->prcpc[i], &atttbl->chem_ic[i][SOIL_CHMVOL]);
#endif
    ...
    for (i = 0; i < chmictbl->nic; i++)
    {
        ...
        for (k = 0; k < num_stc; k++)
        {
            ...
            {
                if (sscanf(cmdstr, "%*s %lf %*s %lf", &chmictbl->conc[i][ind], &chmictbl->ssa[i][ind]) !=2)
                {
                    pihm_printf(VL_ERROR, "Error reading initial condition in %s at Line %d.\n", fn, lno);
                }
            }
            else
            {
            ...
            chmictbl->conc[i][ind] = (strcmp(chemtbl[ind].name, "pH") == 0 && convert == 1) ?
