...
void SoluteConc(const chemtbl_struct chemtbl[], const rttbl_struct *rttbl, elem_struct elem[], river_struct river[])
{
    int             i;
...
# pragma omp parallel for
    ...
    for (i = 0; i < nelem; i++)
    {
        ...
        for (k = 0; k < nsolute; k++)
        {
            elem[i].solute[k].conc_surf = elem[i].prcpchm.tot_conc[k] * rttbl->cond;

            // Calculate concentrations
            elem[i].solute[k].conc = (storage > DEPTHR) ? elem[i].chms.tot_mol[k] / storage : 0.0;
                ...
                for (kk = 0; kk < rttbl->num_ssc; kk++)
                {
                        ...
                        (chemtbl[kk + rttbl->num_stc].itype != AQUEOUS))
                    {
                        elem[i].solute[k].conc -=
                            rttbl->conc_contrib[k][kk + rttbl->num_stc] * elem[i].chms.sec_conc[kk];
                    }
                }
            }
        ...
        {
            // Calculate concentrations
            elem[i].solute[k].conc_geol = (storage > DEPTHR) ? elem[i].chms_geol.tot_mol[k] / storage : 0.0;
                ...
                for (kk = 0; kk < rttbl->num_ssc; kk++)
                {
                        ...
                        (chemtbl[kk + rttbl->num_stc].itype != AQUEOUS))
                    {
                        elem[i].solute[k].conc_geol -=
                            rttbl->conc_contrib[k][kk + rttbl->num_stc] * elem[i].chms_geol.sec_conc[kk];
                    }
                }
            }
...
# pragma omp parallel for
    ...
    for (i = 0; i < nriver; i++)
    {
        ...
        {
            // Calculate concentrations
            river[i].solute[k].conc = (storage > DEPTHR) ? river[i].chms.tot_mol[k] / storage : 0.0;
